...
def csv2dict(anno_path, dataset_type):
    ...
    for file_idx in range(len(inputs_list)):
        ...
        info_dict[file_idx] = {
            ...
            'signer': signer,
            'fsw': fsw,
            'label': fsw, # this can be changed to gloss dep on use-case
            'gloss': gloss,
            'num_frames': num_frames,
            'original_info': inputs_list.loc[file_idx].to_dict(),
        }
    return info_dict
...
def resize_img(img_path, dsize='1920x1080px'):
    # print('RESIZE IMG', dsize)
...
def resize_dataset(video_idx, dsize, info_dict):
    ...
    img_list = glob.glob(f"{info['folder']}/*")
    # print(info['folder'], img_list)
    for img_path in img_list:
        rs_img = resize_img(img_path, dsize=dsize)
        # rs_img_path = img_path.replace("210x260px", dsize)
        rs_img_path = img_path.replace("1920x1080px", dsize)
        # print('img path:', rs_img_path)
        rs_img_dir = os.path.dirname(rs_img_path)
        # print('img dir:', rs_img_dir)
